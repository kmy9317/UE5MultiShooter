...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (PathResults.HitResult.bBlockingHit) // we hit the head, return early
	{
		if (PathResults.HitResult.Component.IsValid())
		{
			UBoxComponent* Box = Cast<UBoxComponent>(PathResults.HitResult.Component);
			if (Box)
			{
				DrawDebugBox(GetWorld(), Box->GetComponentLocation(), Box->GetScaledBoxExtent(), FQuat(Box->GetComponentRotation()), FColor::Red, false, 8.f);
			}
	...
	else // we didn't hit the head, check the rest of the boxes
	{
		...
		UGameplayStatics::PredictProjectilePath(this, PathParams, PathResults);
		if (PathResults.HitResult.bBlockingHit)
		{
			if (PathResults.HitResult.Component.IsValid())
			{
				UBoxComponent* Box = Cast<UBoxComponent>(PathResults.HitResult.Component);
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	// Enable collsion for boxes, then disable for head boxes
	for (auto& Frame : FramePackages)
	{
...
void ULagCompensationComponent::CachedBoxPosition(ABlasterCharacter* HitCharacter, FFramePackage& OutFramePackage)
{
